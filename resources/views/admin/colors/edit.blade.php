...
@section('title', 'Editar Cor - Lumez')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <div class="flex items-center gap-2">
                    <input type="text" name="hex_code" value="{{ old('hex_code', $color->hex_code) }}" placeholder="#000000"
                           pattern="^#[0-9A-Fa-f]{6}$"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
                    @if($color->hex_code)
                        <span class="w-12 h-12 rounded-lg border border-gray-300" style="background-color: {{ $color->hex_code }}"></span>
                    @endif
                </div>
            ...
            <div>
                ...
                <input type="number" name="order" value="{{ old('order', $color->order ?? 0) }}"
                       class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
            </div>
        </div>
